...
```{r}
sentimentagg <- sentiment1 %>%
  filter(movie == "The Fellowship of the Ring ") %>%
  group_by(char)%>%
  summarise('meanSentiment' = mean(SentimentGI, na.rm = TRUE))
...
data <- data %>% 
  mutate(char = str_to_upper(name))

FOTRwithSentiment <- data %>% right_join(sentimentagg,by = 'char')
